...
```{r}
# unique(loaded_base_data$Tournament)
# a <- loaded_base_data %>% 
#   group_by(TournamentFile) %>% 
#   # mutate(min_wins = min(Wins,na.rm = TRUE)) %>% 
#   # filter(str_detect(Tournament,
#   #              "Modern Challenge \\d+|Modern Preliminary|Modern Showcase Challenge|Modern Super Qualifier|Modern Last Chance|Modern Showcase Qualifier"))%>% view()
#   # rowwise() 
#   mutate( a =  if_else(
#     str_detect(Tournament,
#                "Modern Challenge \\d+|Modern Preliminary|Modern Showcase Challenge|Modern Super Qualifier|Modern Last Chance|Modern Showcase Qualifier"),
#     Wins - min(Wins,na.rm = TRUE),Wins
#     
#     ),.before = Wins
#     
#     ) 
...
# 
# 
# a %>% group_by(TournamentFile) %>% 
#   summarise(winrate_base = sum(Wins,na.rm = TRUE)/(sum(Losses,na.rm = TRUE)+sum(Wins,na.rm = TRUE)),
#             patch_mtg0 = sum(a,na.rm = TRUE)/(sum(Losses,na.rm = TRUE) + sum(a,na.rm = TRUE))
#             
#             ) %>% 
#   mutate(confirm = winrate_base== patch_mtg0) %>% 
#   
#   view()
```
