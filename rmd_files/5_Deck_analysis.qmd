...
named_and_groups_combo <-   
  linear_combo_definition(fun_list_of_linear_comb = linear_combo) %>% 
    # check whether any numbers of an element are in any of the elements
    # map(~map_lgl(linear_combo, purrr::compose(any, `%in%`), .x)) %>% 
    # unique() %>%    # drop duplicated groups
...
```{r base_arch_agreg_land_name}
df_Base_Archetype_long <- df_export %>%
  mutate(Archetype = Base_Archetype) %>% 
  filter(!is.na(Wins)) %>%
  filter(Losses + Wins > 0) %>% 
  rowwise() %>%
  filter(!is.null(Mainboard)) %>%
...
Df_Base_Archetype_cards_land_name_agreg <- rbind(
  df_Base_Archetype_long %>%
    group_by(Archetype) %>%
    mutate(Archetype_count = n_distinct(id)) %>%
    mutate(
      Mainboard_CardName =
...
Df_Base_Archetype_cards_agreg <- Df_Base_Archetype_cards_land_name_agreg %>%
  group_by(Archetype, Archetype_count, Mainboard_CardName, Mainboard_Count) %>%
  summarise(
    Wins = sum(Wins),
    Losses = sum(Losses),
...
```{r}
Deck_Base_Archetype_win_rate_join <- df_Base_Archetype_long %>%
  ungroup() %>%
  distinct(id,.keep_all = TRUE) %>% 
  group_by(Archetype) %>%
  summarise(Archetype_winrate = sum(Wins,na.rm = TRUE)/(sum(Wins,na.rm = TRUE) + sum(Losses,na.rm = TRUE))
              ) %>% 
  ungroup()
```
...
```{r agreg_sideboard_base_arch}
df_side_Base_Archetype_long <- df_export %>%
    mutate(Archetype = Base_Archetype) %>% 
  # remove deck with less than 60 cards or no side / no main or with not enought data in arch
  filter(id %in% unique(df_Base_Archetype_long$id)) %>%
  group_by(Archetype) %>%
  mutate(
    Archetype_count = n()
  ) %>%
...
Df_side_Base_Archetype_cards_agreg <- df_side_Base_Archetype_long %>%
  group_by(Archetype, Archetype_count, Sideboard_CardName, Sideboard_Count) %>%
  summarise(
...
```{r uncommon_cards_base_arch}
Uncommon_Base_Archetype_cards_pre_process <- Df_Base_Archetype_cards_agreg %>%
  # filter(count_iteration_cards == most_common_count) %>%
  filter(
    (Archetype_count - most_common_count) >= min_sample_size_5,
    # most_common_count >= min_sample_size_5
...
Uncommon_Base_Archetype_cards_agreg_out <- Agreg_count_by_cards(
  Uncommon_Base_Archetype_cards_pre_process,
  "Mainboard",
  min_sample_size_5
)

Uncommon_Base_Archetype_cards <- Uncommon_Base_Archetype_cards_agreg_out %>%
  group_by(Archetype, Mainboard_CardName)
```



```{r Side_uncommon_cards_base_arch}
Side_Uncommon_Base_Archetype_cards_pre_process <- Df_side_Base_Archetype_cards_agreg %>%
  # filter(count_iteration_cards == most_common_count) %>%
  filter(
    (Archetype_count - most_common_count) >= min_sample_size_5,
    # most_common_count >= min_sample_size_5
...
Side_Uncommon_Base_Archetype_cards_agreg_out <- Agreg_count_by_cards(
  Side_Uncommon_Base_Archetype_cards_pre_process,
  "Sideboard",
  min_sample_size_5
)

Side_Uncommon_Base_Archetype_cards <- Side_Uncommon_Base_Archetype_cards_agreg_out %>%
  group_by(Archetype, Sideboard_CardName)
```
...
```{r Base_cards_grouping_with_agreg_base_arch}
# Récupération des cartes a 1 niveaux après agreg
Base_Archetype_Base_cards_and_base_count <- 
  Df_Base_Archetype_cards_agreg %>%
    mutate(
      min_count_group = Mainboard_Count,
      most_common_quantity = Mainboard_Count
    ) %>%
  ...
  mutate(Mainboard_Count = paste0(findIntRuns(as.numeric(unlist(str_split(Mainboard_Count, "/")))), collapse = "/")) %>%
  ungroup() %>%
  left_join(Deck_Base_Archetype_win_rate_join,by = "Archetype") %>% 
  group_split(Archetype) %>%
  name_list_of_df_with_arch()
```


```{r Side_Base_cards_grouping_with_agreg_base_arch}
# Récupération des cartes a 1 niveaux après agreg
Base_Archetype_Side_Base_cards_and_base_count <-
  Df_side_Base_Archetype_cards_agreg %>%
    mutate(
      min_count_group = Sideboard_Count,
      most_common_quantity = Sideboard_Count
    ) %>%
...
```{r model_unco_cards_base_arch}
Model_Base_Archetype_data_Uncommon_cards <- prepare_df_for_model(
  df_fun = Uncommon_Base_Archetype_cards,
  base_df = Df_Base_Archetype_cards_land_name_agreg,
  cols_fun = "Mainboard"
  ) 
...
```{r}
Base_Archetype_groupe_cards_uncommon_data <- group_linear_comb_cards(
  df_long = Model_Base_Archetype_data_Uncommon_cards
  )
```
...
```{r}
# Projet avec les images des cartes pour arborescence deck list penser a mettre en gras le compte le plus rreprésenter pour base card and base count
result_models_Uncommon_cards_all_Base_Archetype <- model_unco_cards_fun(
  df_fun = Base_Archetype_groupe_cards_uncommon_data
  ) %>%
  name_list_of_model_with_string(unique(Base_Archetype_groupe_cards_uncommon_data$Archetype))
...
```{r format_main_models_base_arch}
Base_Archetype_uncomon_card_format_model <- format_model_list(result_models_Uncommon_cards_all_Base_Archetype) %>%
  name_list_of_model_with_string(unique(Base_Archetype_groupe_cards_uncommon_data$Archetype))
...
```{r side_model_unco_cards_base_arch}
Base_Archetype_Side_Model_data_Uncommon_cards <-  prepare_df_for_model(
  df_fun = Side_Uncommon_Base_Archetype_cards,
  base_df = df_side_Base_Archetype_long,
  cols_fun = "Sideboard"
  )
# Projet avec les images des cartes pour arborescence deck list penser a mettre en gras le compte le plus rreprésenter pour base card and base count
```
...
```{r}
Base_Archetype_groupe_side_cards_uncommon_data <- group_linear_comb_cards(Base_Archetype_Side_Model_data_Uncommon_cards)
```
...
```{r}
Side_result_models_Uncommon_cards_all_Base_Archetype <- model_unco_cards_fun(
  df_fun = Base_Archetype_groupe_side_cards_uncommon_data
  ) %>%
  name_list_of_model_with_string(unique(Base_Archetype_groupe_side_cards_uncommon_data$Archetype))
```


```{r format_side_models_Base_Archetype}
Base_Archetype_Side_uncomon_card_format_model <- format_model_list(Side_result_models_Uncommon_cards_all_Base_Archetype) %>%
  name_list_of_model_with_string(unique(Base_Archetype_groupe_side_cards_uncommon_data$Archetype))
```
...
```{r print_number_df_base_arch}
Df_Base_Archetype_base_number_to_print <- Df_Base_Archetype_cards_land_name_agreg %>% 
  ungroup() %>%
  distinct(id,.keep_all = TRUE) %>% 
  group_by(Archetype,Archetype_count) %>% 
  summarise(
...
if(is_inside_knitr()){
for (i in sort(levels(df_Base_Archetype_long$Archetype))) {
  
  temp_df_to_print <- Df_Base_Archetype_base_number_to_print %>%
    filter(Archetype == i)
  ...
  if (i %in% names(Base_Archetype_Base_cards_and_base_count)) {
  pander::pandoc.header("Base Cards", level = 4)
  pander::pandoc.p("")
  pander::pandoc.p("Cards Always in deck with nearly fix count")
  pander::pandoc.p(print_count_string)
  ...
  pander::pandoc.p("")
    flextable::flextable_to_rmd(
      flextable::flextable(
        Base_Archetype_Base_cards_and_base_count[[i]] %>%
          mutate(
            WR = paste0(round(((Wins  / (Wins + Losses)) - Archetype_winrate)* 100, 2), " %"),
            Not_most_common_count = total_number_of_copie - most_common_count,
            Card_not_in_deck = Archetype_count - total_number_of_copie
  ...
  if (i %in% names(Base_Archetype_uncomon_card_format_model)) {
  pander::pandoc.header("Variable Cards", level = 4)
  pander::pandoc.p("")
  pander::pandoc.p("Cards not always in deck using binomial regression for WR")
  pander::pandoc.p("")
    pander::pandoc.p('::: {.panel-tabset .nav-pills}')
if (!is.null(Base_Archetype_uncomon_card_format_model[[i]]$Model_any)) {
    pander::pandoc.header("Any", level = 5)
    flextable::flextable_to_rmd(
      Base_Archetype_uncomon_card_format_model[[i]]$Model_any %>%
        gtsummary::as_flex_table()
    )
}
    pander::pandoc.p("")
    ...
    # pander::pandoc.p('::: {.column width="50%"}')
    pander::pandoc.p("")
    if (!is.null(Base_Archetype_uncomon_card_format_model[[i]]$Model_count )) {
    pander::pandoc.header("Count", level = 5)
    flextable::flextable_to_rmd(
      Base_Archetype_uncomon_card_format_model[[i]]$Model_count %>%
        gtsummary::as_flex_table()
    )
    }
    pander::pandoc.p("")
    if (!is.null(Base_Archetype_uncomon_card_format_model[[i]]$model_ridge )) {
     pander::pandoc.header("Ridge", level = 5)
    # flextable::flextable_to_rmd(
      print(htmltools::tagList(Base_Archetype_uncomon_card_format_model[[i]]$model_ridge))# %>%        gt::as_flex_table()
    # )
}
#     pander::pandoc.p(":::
# ::::::::::::::")
  ...
  if (i %in% names(Base_Archetype_Side_Base_cards_and_base_count)) {
  pander::pandoc.header("Base Cards", level = 4)
  pander::pandoc.p("")
  pander::pandoc.p("Cards Always in deck with nearly fix count")
  pander::pandoc.p(print_count_string)
  pander::pandoc.p("")
   flextable::flextable_to_rmd(
      flextable::flextable(
        Base_Archetype_Side_Base_cards_and_base_count[[i]] %>%
          mutate(
            WR = paste0(round(((Wins  / (Wins + Losses)) - Archetype_winrate)* 100, 2), " %"),
            Not_most_common_count = total_number_of_copie - most_common_count,
            Card_not_in_deck = Archetype_count - total_number_of_copie
...
# ::: {.column width="50%"}')
    ...
    pander::pandoc.p('::: {.panel-tabset .nav-pills}')
    if (!is.null(Base_Archetype_Side_uncomon_card_format_model[[i]]$Model_any)) {
       pander::pandoc.header("Any", level = 5)
    flextable::flextable_to_rmd(
      Base_Archetype_Side_uncomon_card_format_model[[i]]$Model_any %>%
        gtsummary::as_flex_table()
    )
}
    # pander::pandoc.p(":::")
    ...
    # pander::pandoc.p('::: {.column width="50%"}')
    pander::pandoc.p("")
if (!is.null(Base_Archetype_Side_uncomon_card_format_model[[i]]$Model_count)) {
     pander::pandoc.header("Count", level = 5)
    flextable::flextable_to_rmd(
      Base_Archetype_Side_uncomon_card_format_model[[i]]$Model_count %>%
        gtsummary::as_flex_table()
    )
} 
    pander::pandoc.p("")
    if (!is.null(Base_Archetype_Side_uncomon_card_format_model[[i]]$model_ridge )) {
     pander::pandoc.header("Ridge", level = 5)
    # flextable::flextable_to_rmd(
      print(htmltools::tagList(Base_Archetype_Side_uncomon_card_format_model[[i]]$model_ridge))# %>%        gt::as_flex_table()
    # )
}
#     pander::pandoc.p(":::
# ::::::::::::::")
