...
```{r}
df_arch_init <- (groupe_cards_uncommon_data  %>%
  ungroup() %>%
  pivot_wider(
...
# 
if(!is_inside_knitr()){
df_base_fun <- groupe_cards_uncommon_data

col_en_cours <- df_base_fun %>%
    select(ends_with("_CardName")) %>%
    colnames() %>%
    str_remove("_CardName")


df_to_join <- df_base_fun %>%
  select(
    Archetype,id,Archetype_count,
         all_of(c(paste0(col_en_cours, "_CardName"),
                  paste0(col_en_cours, "_Count")))
         ) %>%
  filter(!str_detect(!!rlang::sym(paste0(col_en_cours, "_CardName"))," land$"),
         !!rlang::sym(paste0(col_en_cours, "_CardName")) != "Land_tot") %>% 
  group_by(Archetype, !!rlang::sym(paste0(col_en_cours, "_CardName"))) %>%
  mutate(
    total_card = n()
    ) %>%
  group_by(Archetype, !!rlang::sym(paste0(col_en_cours, "_CardName")),
           !!rlang::sym(paste0(col_en_cours, "_Count"))) %>%
  mutate(
    count_iteration_card = n()
    )



collapse_col_df <- left_join(
 df_to_join,
  df_to_join,
 by = join_by(id,Archetype,Archetype_count
              ),
  relationship = "many-to-many"
  ) %>%
  filter(!(
    !!rlang::sym(paste0(col_en_cours, "_CardName.x")) == !!rlang::sym(paste0(col_en_cours, "_CardName.y")) &
      !!rlang::sym(paste0(col_en_cours, "_Count.x")) == !!rlang::sym(paste0(col_en_cours, "_Count.y"))
    )
    ) %>%
  # rowwise() %>%
  # mutate(
  #   key = paste0(sort(c(
  #     paste0(!!rlang::sym(paste0(col_en_cours, "_CardName.x")),
  #            !!rlang::sym(paste0(col_en_cours, "_Count.x"))),
  #     paste0(!!rlang::sym(paste0(col_en_cours, "_CardName.y")),
  #            !!rlang::sym(paste0(col_en_cours, "_Count.y")))
  #     )), collapse="")
  #   ) %>%
  #  ungroup() %>%
  #  distinct(id,Archetype,key,.keep_all = TRUE) %>%
  #  select(-key) %>%
    mutate(
    !!rlang::sym(paste0(col_en_cours, "_CardName")) :=
      paste0(
        !!rlang::sym(paste0(col_en_cours, "_CardName.x")),
        " ; " ,
        !!rlang::sym(paste0(col_en_cours, "_CardName.y"))
             ),
    !!rlang::sym(paste0(col_en_cours, "_Count")) :=
      paste0(
        !!rlang::sym(paste0(col_en_cours, "_Count.x")) ,
...
if(!is_inside_knitr()){
df_two_way_cards_combination_summary <- collapse_col_df %>%
  group_by(Archetype,Mainboard_CardName) %>%
  mutate(
    n_card_combination =  n()
  ) %>%
  ungroup() %>%
  group_by(Archetype,Mainboard_CardName,Mainboard_Count) %>%
  mutate(
    n_card_count_combination = n()
    ) %>% 
  ungroup() %>% 
  select(-id) %>% 
  distinct() %>%
  mutate(
    cover_perc.x = n_card_count_combination/count_iteration_card.x,
    cover_perc.y = n_card_count_combination/count_iteration_card.y,
    cover_association.x = n_card_combination/total_card.x,
    cover_association.y = n_card_combination/total_card.y,
    remaining.x =  count_iteration_card.x - n_card_count_combination ,
    remaining.y = count_iteration_card.y - n_card_count_combination,
    remaining_total.x = total_card.x - n_card_combination,
    remaining_total.y = total_card.y - n_card_combination
  ) 



a <- df_two_way_cards_combination_summary %>% 
   # filter(Archetype == "RWx aggro") %>% 
  filter(cover_association.x == 1,
         cover_perc.x == 1,
         remaining_total.y >= 50,
         remaining.y >= 50
         )  %>%
  group_by(Archetype,Mainboard_CardName,Mainboard_Count) %>%
  filter(
    sum(count_iteration_card.x) == total_card.x
  ) %>%
  ungroup() 
