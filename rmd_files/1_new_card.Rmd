...
if (refetch_cards_new_set){
  ...
  cards_in_set_of_interest <- scry_cards("e:mh3")
...
card_of_interest <- modern_DB_card %>%
  filter(name %in% cards_in_set_of_interest_name,
...
df_new_card_main_base <- df_export %>%
...
```{r}
Bulk_new_cards <- rbind( df_new_card_main_base  %>% 
  select(Mainboard_Count,Mainboard_CardName,id) %>% 
  rename(Count = 1 , CardName = 2) ,
  df_export %>%
  unnest_longer(Sideboard) %>%
  unnest_wider(Sideboard, names_sep = "_") %>%
  filter(Sideboard_CardName %in% card_of_interest$name) %>% 
  select(Sideboard_Count,Sideboard_CardName,id) %>% 
  rename(Count = 1 , CardName = 2)
  ) %>% 
  group_by(CardName,id) %>% 
  summarise(
    Count = sum(Count) ,
    .groups = "drop"
    ) %>% 
...
already_save <- read.delim(
  "../data/save_mh3_cards.txt",
  header = FALSE ,
  blank.lines.skip = FALSE
  ) %>% 
  mutate(
    V1 = trimws(str_remove(V1,"^\\d+"))
  )

tresh_hold_to_put_in_file <- 10
  
write_new_cards_df <- Bulk_new_cards %>%
      filter(Count > tresh_hold_to_put_in_file,
             !(CardName %in% already_save$V1)) 
...
writeLines(
  paste0(
   ...
   write_new_cards_df %>%
      pull(CardName)),
  "../data/intermediate_result/played_mh3_cards.txt")
