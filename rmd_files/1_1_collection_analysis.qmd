...
path_collection <- 
  list.files(
      path = "../data/",pattern = "^archidekt-collection-export-",
               full.names = TRUE)
```
...
Summary_missing_cards_by_archetype <-
 ...
 # print(
  ...
  DT::formatPercentage(c("Percent_of_deck_with_missing_cards"), 1)
)
     # )
```
...
not_own_car_by_archetype_without_any <- not_owned_cards %>%
  ...
  mutate(
    Archetype_count = 
      round(n_quant_arch/Archetype_count,1)
  ) %>% 
  distinct() %>% 
    ungroup() %>% 
  select(-missing) %>% 
...
for (i in Archetype_list) {
  ...
  DT::formatPercentage(c("Archetype_count"), 1)
)
    )
    pander::pandoc.p("")
    pander::pandoc.p("")
...
```{r}
 # print(
  ...
  DT::formatPercentage(c("Base_Archetype_count"), 1)
)
    # )
