...
#| column: screen
    ...
    num_archetypes <- length(levels(spaghetti_plot_df$Archetype))
    ...
    archetype_levels <- levels(spaghetti_plot_df$Archetype)
...
num_base_archetypes <- length(levels(spaghetti_plot_df$Base_Archetype))
colors_scales_base_arch_presence <- scheme$hex(ceiling(num_base_archetypes / 2))  # Diviser le nombre de couleurs par 2

# Associer chaque Archetype avec une couleur et un style de ligne
base_archetype_levels <- levels(spaghetti_plot_df$Base_Archetype)
...
Archetype_ci_df_plot_lines_df <- data.frame(
    yintercept = c(
        mean(df_Ci_by_all_week$Arch_winrate),
...
(ggplot(data = df_Ci_by_all_week,aes( 
  text = paste(
  ...
  ggplotly(
    tooltip = c("text"),
    height = 50 * nrow(df_Ci_by_all_week)
...
base_Archetype_ci_df_plot_lines_df <- data.frame(
    yintercept = c(
        mean(df_base_arch_Ci_by_all_week$Arch_winrate),
...
(ggplot(data = df_base_arch_Ci_by_all_week,
  ...
  ggplotly(
    tooltip = c("text"),
    height = 50 * nrow(df_base_arch_Ci_by_all_week)
    # height = (480 * ratio_plot), width = (820 * ratio_plot)
  )  %>%
  plotly::layout(
    legend = list(x = 0.85, y = 0.15) # Position précise de la légende dans le graphique Plotly
...
escape = FALSE
  ...
  DT::formatPercentage(c(7,9,12#,10,13
                         ),digits = 1)
```
