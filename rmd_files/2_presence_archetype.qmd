...
spaghetti_plot_df <- Presence_df_base %>%
  ...
  mutate(
    Week = factor(
      Week,
      levels = c(1:max(Presence_df_base$Week))
    )
  )
...
modern_db_join_with_castable_number_of_deck <-   modern_DB_card %>% 
        select(id,set,scryfall_uri,starts_with("colors.")) %>%
  pivot_longer(cols = starts_with("colors"), names_to = "colnames", values_to = "color") %>%
  filter(!is.na(color)) %>%
  mutate(
    color_W = ifelse(color == "W", 1, 0),
    color_B = ifelse(color == "B", 1, 0),
    color_U = ifelse(color == "U", 1, 0),
    color_R = ifelse(color == "R", 1, 0),
    color_G = ifelse(color == "G", 1, 0)
  ) %>%
  # Ajouter une ligne pour les cartes incolores
  bind_rows(modern_DB_card %>% 
        select(id,set,scryfall_uri,starts_with("colors.")) %>% 
          filter(is.na(colors.1)) %>% 
          mutate(
    color_W = 0, color_B = 0, color_U = 0, color_R = 0, color_G = 0
  )) %>%
...
number_of_copy_of_each_cards_final_table <-  rbind(
  ...
  relocate(winrate_count_castable,.after = count_presence_percent) %>% 
...
escape = FALSE
  ...
  DT::formatPercentage(c(7,10,12,13,16
                         #,10,13
  ),digits = 1)
```
