...
::: {.content-visible when-profile="collection_tracking"}
...
```{r}
basic_land_list <- c(
  "Swamp", "Mountain", "Plains", "Island", "Forest"
  )

Deck_list_cards_count <-
  rbind( df_main_unnest  %>%
...
collection_archi <- 
  read.csv(
    "../data/archidekt-collection-export-2024-08-05.csv"
    ) %>% 
  group_by(Name) %>% 
...
not_owned_cards <- Deck_list_cards_count %>% 
  left_join(
    collection_archi,
    by = c("CardName" = "Name")
    ) %>% 
...
# Cards in deck and not in collection 
...
for (i in Archetype_list) {
  ...
  not_owned_cards %>%
  filter(Archetype == i) %>% 
  select(-c(Archetype,Base_Archetype,Base_Archetype_count,n_quant_base_arch)) %>% 
  group_by(
    CardName,
    Quantity
    ) %>% 
  mutate(
    ...
    Archetype_count = paste0(
      round(n_quant_arch*100/Archetype_count,1),
      "(",Archetype_count,")")
  ) %>% 
  distinct() %>% 
    ungroup() %>% 
...
```{r, results='asis'}
Base_Archetype_list <- df_export %>%
  # filter(Archetype_count > 20) %>% 
  pull(Base_Archetype) %>%
  unique() %>% 
  sort()

for (i in Base_Archetype_list) {
  # Inserts Month titles
  # Section contents
  pander::pandoc.header(i, level = 2)
  pander::pandoc.p("")
  pander::pandoc.p("")
  if (i %in% not_owned_cards$Base_Archetype) {
    print(
      htmltools::tagList(
      DT::datatable(
  not_owned_cards %>%
  filter(Base_Archetype == i) %>% 
  select(-c(Archetype,Base_Archetype,Archetype_count,n_quant_arch)) %>% 
  group_by(
    CardName,
    Quantity
    ) %>% 
  mutate(
    ...
    Base_Archetype_count = paste0(
      round(n_quant_base_arch*100/Base_Archetype_count,1),
