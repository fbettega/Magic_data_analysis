...
#   select(Mainboard_Count,Mainboard_CardName,Archetype,id) %>% 
#   rename(Count = 1 , CardName = 2) %>% 
#   mutate(main_side = "Main"),
#   df_export %>%
#   unnest_longer(Sideboard) %>%
#   unnest_wider(Sideboard, names_sep = "_") %>%
#   select(Sideboard_Count,Sideboard_CardName,Archetype,id) %>% 
#   rename(Count = 1 , CardName = 2) %>% 
...
# 
# a <- Bulk_new_cards %>% 
#   group_by(Archetype,CardName,main_side,Count) %>% 
#   summarise(
#     nb_ite = n() ,
#     .groups = "drop"
#     ) 
