...
Df_win_rate_matrix_base_arch_format <- Df_win_rate_matrix_base_arch_summarise %>%
  mutate(
    number_format_matches = paste0("n = ", number_of_matches,
                                   " [", Win_matches, "-", 
                                   (number_of_matches - Win_matches), "]"),
    CI_WR_format_matches = paste0(round(WR_matches * 100, 1),
                                  "%",
                                  formating_CI(WR_matches,
                                               CI_WR_matches,
                                               round_val = 1,
                                               limit = c(0, 1))
                                  ),
...
Top_player_base_df <- Df_win_rate_matrix_base %>% 
  mutate(Wins = Matchups_Wins > Matchups_Losses,
         Losses  = Matchups_Wins < Matchups_Losses
         ) %>% 
  group_by(Archetype,Base_Archetype,Player) %>% 
  summarise(
    Wins = sum(Wins),
    Losses = sum(Losses),
    .groups = "drop"
...
```{r One_week_base_data}
One_month_week_vector <- Df_win_rate_matrix_base %>% 
  filter(Date >= as.Date(Sys.time()) %m-% months(1)) %>% 
  distinct(Week) %>% 
  pull(Week)
  

Df_win_rate_matrix_base_1_month <- Df_win_rate_matrix_base %>% 
  filter(Week %in% One_month_week_vector) 

Top_player_base_1_month_df <- Df_win_rate_matrix_base_1_month %>% 
  mutate(Wins = Matchups_Wins > Matchups_Losses,
         Losses  = Matchups_Wins < Matchups_Losses
         ) %>% 
  group_by(Archetype,Base_Archetype,Player) %>% 
...
```{r winrate_matrix_arch_1_month}
Df_win_rate_matrix_format_1_month <-  win_rate_matrix_maker(
  df_fun = Df_win_rate_matrix_base_1_month,
  Archetype_type = "Archetype" # "Archetype" , "Base_Archetype"
) %>%
  mutate(
    number_format_matches = paste0("n = ", number_of_matches, " [", Win_matches, "-", (number_of_matches - Win_matches), "]"),
...
```{r winrate_matrix_base_arch_1_month}
Df_win_rate_matrix_base_arch_format_1_month <-  win_rate_matrix_maker(
  df_fun = Df_win_rate_matrix_base_1_month,
  Archetype_type = "Base_Archetype" # "Archetype" , "Base_Archetype"
) %>%
  mutate(
...
* One month data : `r format(as.Date(min(Df_win_rate_matrix_base_1_month$Date)), "%d-%m-%Y")` to `r format(as.Date(max(Df_win_rate_matrix_base_1_month$Date)), format = "%d-%m-%Y")`. 
* Duration `r round(max(difftime(Sys.Date(),Df_win_rate_matrix_base_1_month$Date,units = "weeks")),0)` weeks.
* Include meta : `r unique(Df_win_rate_matrix_base_1_month$Meta)`.  
* Number of tournament : `r n_distinct(Df_win_rate_matrix_simple_unnest_matchup %>% filter(Week %in% One_month_week_vector) %>% pull(TournamentFile))`
* With a total of `r nrow(Df_win_rate_matrix_base_1_month)` rounds of magic
* `r nrow(Df_win_rate_matrix_base_1_month %>% distinct(Archetype,Matchups_OpponentArchetype))` different matchup for archetypes.  
* `r nrow(Df_win_rate_matrix_base_1_month %>% distinct(Base_Archetype,Matchups_OpponentBase_Archetype))` different matchup for base archetypes.  
...
```{r presence_plot_1_month}
# <!-- #| column: screen -->
plot_presence_modify_for_matchup_matrix_fun(
  df_base = Df_win_rate_matrix_base_1_month,
  color_scheme =  scheme$hex(
    length(levels(Df_win_rate_matrix_base_1_month$Base_Archetype))
  ),
  plot_scaling = 2.25
) %>%
  bslib::card(full_screen = TRUE)
...
# <!-- #| column: screen -->
plot_win_rate_mat(
  Df_winrate_format_fun = Df_win_rate_matrix_format,
  group_column =  "matches"
  ) 
```
...
```{r res_arch_matrix_1_month}
# <!-- #| column: screen -->
plot_win_rate_mat(
  Df_winrate_format_fun = Df_win_rate_matrix_format_1_month,
  group_column =  "matches"
)
```
...
```{r res_ci_plot_arch}
# <!-- #| column: screen -->
Archetype_ci_df_plot <- CI_plot_prepare_df_fun(
  df_fun = Df_win_rate_matrix_format
  ) %>%
  mutate(
    Archetype = factor(
      Archetype,
      levels = .$Archetype[order(.$Arch_winrate_matches + .$CI_Arch_winrate_matches)]),
    Number_of_match = number_of_matches,
   Archetype_count = tota_number_of_deck_in_arch 
  ) %>% 
  arrange(Archetype) 
...
Archetype_best_player <- best_player_result_fun(
    ...
    df_fun_top_player = Top_player_base_df 
) 
...
```{r res_ci_plot_arch_1_month}
# <!-- #| column: screen -->
Archetype_ci_one_month_df_plot <- CI_plot_prepare_df_fun(
  df_fun = Df_win_rate_matrix_format_1_month
) %>%
  mutate(
    Archetype = factor(
      Archetype,
...
Archetype_best_player_1_month <- best_player_result_fun(
    top_n_player_fun = top_n_player,
    Arch_or_base_arch = "Archetype", #"Base_Archetype" "Archetype"
    df_fun_top_player = Top_player_base_1_month_df
) 

arch_ci_plot_one_month_final <- Generate_CI_plot_fun(
  df_ci_fun_param = Archetype_ci_one_month_df_plot,
  win_rate_fun_par = "Arch_winrate_matches",
  CI_fun_par = "CI_Arch_winrate_matches",
  Arch_or_base_arch = "Archetype", #"Base_Archetype" "Archetype"
  best_player_df_par_fun = Archetype_best_player_1_month
)

arch_ci_plot_one_month_final
```
:::
...
# <!-- #| column: screen -->
plot_win_rate_mat(
  Df_winrate_format_fun = Df_win_rate_matrix_base_arch_format, 
          group_column =        "matches"
  )
```
...
```{r res_base_arch_matrix_1_month}
# <!-- #| column: screen -->
plot_win_rate_mat(Df_win_rate_matrix_base_arch_format_1_month, "matches")
```
...
base_Archetype_best_player <- best_player_result_fun(
  ...
  rename(Archetype = Base_Archetype)
...
```{r res_ci_plot_base_arch_1_month}
# <!-- #| column: screen -->
base_Archetype_ci_one_month_df_plot <- CI_plot_prepare_df_fun(Df_win_rate_matrix_base_arch_format_1_month) %>%
  mutate(
    Archetype = factor(
      Archetype,
      levels = .$Archetype[order(.$Arch_winrate_matches + .$CI_Arch_winrate_matches)]),
...
base_Archetype_best_player_one_month <- best_player_result_fun(
    top_n_player_fun = top_n_player/2,
    Arch_or_base_arch = "Base_Archetype", #"Base_Archetype" "Archetype"
    df_fun_top_player = Top_player_base_1_month_df
) %>% 
...
base_arch_ci_plot_one_month_final <- Generate_CI_plot_fun(
  df_ci_fun_param = base_Archetype_ci_one_month_df_plot,
  win_rate_fun_par = "Arch_winrate_matches",
  CI_fun_par = "CI_Arch_winrate_matches",
  Arch_or_base_arch = "Archetype", #"Base_Archetype" "Archetype"
  best_player_df_par_fun = base_Archetype_best_player_one_month
)

base_arch_ci_plot_one_month_final
```

:::

# Card winrate by matchup  
only on valid deck and with matchup

```{r card_winrate_by_matchup}
Df_win_rate_matrix_base_with_deck_not_unnest <- 
  Df_win_rate_matrix_simple_unnest_matchup %>%
  select(-Matchups_OpponentArchetype) %>%
  inner_join(
    Df_win_rate_matrix_correct_maccro_archetype_oppo,
  ...
  select(
    id, Meta, Week, Date, Archetype, Base_Archetype, Player,Mainboard,Sideboard,Valide_deck,
    Matchups_Opponent, Matchups_OpponentArchetype, Matchups_OpponentBase_Archetype, Matchups_Wins,
    Matchups_Losses, Matchups_Draws, matches
  ) %>%
  mutate(
  ...
  mutate(
    Wins = as.numeric(Matchups_Wins > Matchups_Losses),
    Losses = as.numeric(Matchups_Wins < Matchups_Losses),
    Draws = as.numeric(Wins == 0 & Losses == 0)
  ) %>% 
  select(-c(Matchups_Wins,Matchups_Losses,Matchups_Draws)) %>% 
  group_by(Base_Archetype,Matchups_OpponentBase_Archetype) %>% 
    mutate(
      base_Archetype_count = n()
    ) %>% 
    ungroup() %>% 
  group_by(Archetype, Matchups_OpponentArchetype ) %>% 
    mutate(
      Archetype_count = n()
    ) %>% 
    ungroup() 
...
```{r Arch_cards_win_rate}
Df_win_rate_matrix_card_winrate_arch_mainboard <-  
  Count_and_winrates_cards_in_decklist_total(
    df = Df_win_rate_matrix_base_with_deck_not_unnest %>% 
      filter(Archetype_count > 10) ,
    group_var = c("Archetype","Matchups_OpponentArchetype"),
    colname_deck_list  = "Mainboard",
    Group_arch_and_base_arch = FALSE
    )

Df_win_rate_matrix_card_winrate_arch_sideboard <- 
  Count_and_winrates_cards_in_decklist_total(
    df = Df_win_rate_matrix_base_with_deck_not_unnest %>% 
      filter(Archetype_count > 10) ,
    group_var = c("Archetype","Matchups_OpponentArchetype"),
...
```{r Base_arch_cards_win_rate}
Df_win_rate_matrix_card_winrate_base_arch_mainboard <- 
  Count_and_winrates_cards_in_decklist_total(
    df = Df_win_rate_matrix_base_with_deck_not_unnest %>% 
      filter(base_Archetype_count > 10) %>% 
      select(-Archetype_count) %>% 
      rename(Archetype_count = base_Archetype_count),
    group_var = c("Base_Archetype","Matchups_OpponentBase_Archetype"),
    colname_deck_list  = "Mainboard",
    Group_arch_and_base_arch = FALSE
    )

Df_win_rate_matrix_card_winrate_base_arch_sideboard <- 
  Count_and_winrates_cards_in_decklist_total(
    df = Df_win_rate_matrix_base_with_deck_not_unnest %>% 
      filter(base_Archetype_count > 10) %>% 
      select(-Archetype_count) %>% 
...
tooltips_deck_base <- c(
  ...
  "Deck opponent Archetype agregated",
...
```{r}
DT::datatable(
    Df_win_rate_matrix_card_winrate_arch_mainboard$Card_always_in_deck_final %>%
      filter(Most_present_Mainboard_Count >= 10) %>% 
    rename_with(
      ~ str_replace(
        str_replace(
          str_replace(
...
add_link_to_a_column(
  ...
  mutate(`Main Card` = as.factor(`Main Card` ),
         `Main Count` = as.factor(`Main Count` )) %>% 
  arrange(
...
```{r}
DT::datatable(
    Df_win_rate_matrix_card_winrate_arch_mainboard$Win_rate_df_final %>% 
  filter(count_WR >= 10) %>%
    rename_with(
      ~ str_replace(
        str_replace(
...
escape = FALSE,
  ...
  DT::formatPercentage(c(5,10,12,15),digits = 1)
```
...
```{r}
DT::datatable(
    Df_win_rate_matrix_card_winrate_arch_sideboard$Card_always_in_deck_final %>%
      filter(Most_present_Sideboard_Count >= 10) %>% 
    rename_with(
      ~ str_replace(
        str_replace(
          str_replace(
...
add_link_to_a_column(
  ...
  mutate(`Side Card` = as.factor(`Side Card` ),
      `Side Count` = as.factor(`Side Count` )) %>% 
  arrange(
    desc(`Archetype count`)
          ),
  filter = "top", rownames = FALSE,
...
```{r}
DT::datatable(
    Df_win_rate_matrix_card_winrate_arch_sideboard$Win_rate_df_final %>% 
  filter(count_WR >= 10) %>%
    rename_with(
      ~ str_replace(
        str_replace(
...
```{r}
DT::datatable(
    Df_win_rate_matrix_card_winrate_base_arch_mainboard$Card_always_in_deck_final %>%
      filter(Most_present_Mainboard_Count >= 10) %>% 
    rename_with(
      ~ str_replace(
        str_replace(
...
```{r}
DT::datatable(
    Df_win_rate_matrix_card_winrate_base_arch_mainboard$Win_rate_df_final %>% 
  filter(count_WR >= 10) %>%
    rename_with(
      ~ str_replace(
        str_replace(
...
```{r}
DT::datatable(
    Df_win_rate_matrix_card_winrate_base_arch_sideboard$Card_always_in_deck_final %>%
      filter(Most_present_Sideboard_Count >= 10) %>% 
    rename_with(
      ~ str_replace(
        str_replace(
...
```{r}
DT::datatable(
    Df_win_rate_matrix_card_winrate_base_arch_sideboard$Win_rate_df_final %>% 
  filter(count_WR >= 10) %>%
    rename_with(
      ~ str_replace(
        str_replace(
...
Df_expected_of_week_sum <- rbind(
  Df_Expected_number_of_win_by_week %>%
    # mutate(Week = format(Week, nsmall = 2)) %>%
    group_by(Archetype, Week) %>%
    summarise(
  ...
  mutate(
    Week = factor(ifelse(Week == 0,"All",format(Week, nsmall = 2)),
              levels = c(
                  format(sort(unique(df_base_all_data$Week)), nsmall = 2),
                  "All"
...
Presence_for_best_deck_plot <- rbind(
  ...
  mutate(
   Week = factor(ifelse(Week == 0,"All",format(Week, nsmall = 2)),
              levels = c(
                  format(sort(unique(Df_Expected_number_of_win_by_week$Week)), nsmall = 2),
                  "All"
                  # as.character(seq(1,max(Presence_df_base$Week)))
              )
) 
...
Presence_Base_Archetype_for_best_deck_plot <- rbind(
  ...
  mutate(
        Week = factor(ifelse(Week == 0,"All",format(Week, nsmall = 2)),
                  levels = c(
                    format(sort(unique(Df_Base_Archetype_Expected_number_of_win_by_week$Week)), nsmall = 2),
                    "All"
                             # as.character(seq(1,max(Presence_df_base$Week)))
                  )
    )
