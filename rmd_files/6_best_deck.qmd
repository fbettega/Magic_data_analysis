...
model_total_cards_fun <- function(df_fun, cols_fun) {
  ...
  model_total_cards_sub_fun <- function(df_fun, cols_fun,archetype_en_cours){
    res <- df_fun %>%
        ...
        select(
          -any_of(c("Archetype", "Player", "Archetype_count",
               "Draws"))
          ) %>% 
...
df_Archetype_long <- df_export %>%
  ...
  select(
    ...
    Mainboard_Count, Mainboard_CardName
         ) %>%
  group_by(id) %>%
...
df_side_Archetype_long <- df_export %>%
  ...
  select(id, Player, Wins, Losses, Draws, Archetype,Color,
         Sideboard_CardName, Sideboard_Count) %>%
  mutate(
    Archetype =
      factor(Archetype,
        level = unique(.$Archetype)
