...
Agreg_count_by_cards <- function(df,
  ...
  df_result <- df %>%
    ...
    mutate(
      ...
      # Gestion compliqué des cas ou les les 2 groupes sont contigu se qui conduisait a des doubles groupement
...
Side_Model_data_base_cards <- Side_Base_cards_and_variable_count %>%
  rowwise() %>%
  mutate(Join_main_count = list(
    as.numeric(unlist(strsplit(Sideboard_Count, "/")))
  )) %>%
  ungroup() %>%
  unnest_longer(Join_main_count) %>%
  select(
    Archetype, Archetype_count, Sideboard_CardName,
    Sideboard_Count, Join_main_count # ,most_common_quantity
  ) %>%
  right_join(
    df_side_Archetype_long,
    by = c("Archetype", "Archetype_count", "Sideboard_CardName", "Join_main_count" = "Sideboard_Count")
  ) %>%
  ungroup() %>%
  filter(!is.na(Sideboard_Count))
...
Side_Model_data_Uncommon_cards <- Side_Uncommon_cards %>%
  rowwise() %>%
  mutate(Join_main_count = list(
    as.numeric(unlist(strsplit(Sideboard_Count, "/")))
  )) %>%
...
```{r print_result_all_data, results='asis', out.width="100%"}
...
```{r print_result_2weeks, results='asis', out.width="100%"}
for (i in sort(unique(res_global$Archetype))) {
  # Inserts Month titles
  # Section contents
  pander::pandoc.header(i, level = 2)
