...
```{r, Introduction_chunk_7_best_deck, results='asis'}
...
top_8_table_and_plot_prez_generator <- function(df_fun,current_tournament){
  ...
  unlist_side_or_main_deck_winner <- function(df,top_x_rank,cols_fun){
    ...
    Unnest_filter_table <- df %>%  
      select(
        cols_fun,not_colfuns,Place,Player,AnchorUri, Archetype,Base_Archetype
      ) %>% 
      select(-not_colfuns) %>% 
...
for (i in seq_along(TournamentFile_by_week)){
  ...
  for (u in seq_along(total_result)){
    if(names(total_result)[u] %in% TournamentFile_by_week[[i]])
      pander::pandoc.header(total_result[[u]]$Title, level = 2)
      pander::pandoc.p("")
      pander::pandoc.header("Presence plot", level = 3)
      print(htmltools::tagList(total_result[[u]]$plot$plot_prez))
      # print(htmltools::tagList(as.widget(total_result[[u]]$plot$plot_prez)))
      # cat(knitr::knit_print(total_result[[u]]$plot$plot_prez, width = 672, height = 480))
      pander::pandoc.p("")
      pander::pandoc.p("")
